    ...
    with open(txtpath, 'r') as f_in:   #打开txt文件          
        ...
        for i, splitline  in enumerate(splitlines):
            ...
            if hbb:
                ...
                box = [x1,y1,x2,y2,x3,y3,x4,y4,label,difficult]
                boxes.append(box)
    return boxes
...
def writeXml(tmp, imgname, w, h, d, bboxes, hbb = False):  
    ...
    for bbox in bboxes:
        ...
        object_new.appendChild(label)
        label_txt=doc.createTextNode(bbox[8])
        label.appendChild(label_txt)
        
        difficult = doc.createElement('difficult')  
        object_new.appendChild(difficult)  
        #difficult_txt = doc.createTextNode("0")  
        difficult_txt = doc.createTextNode(bbox[9])
...
if __name__ == '__main__':
    #data_path = './dota/train'
    data_path = './train'
    images_path = os.path.join(data_path, 'images') #样本图片路径

    # labeltxt_path = os.path.join(data_path, 'labelTxt-v1.5/DOTA-v1.5_train') #DOTA标签的所在路径
    # anno_new_path = os.path.join(data_path, 'obbxml')  #新的voc格式存储位置（obb形式）
...
#感谢源代码作者所提供的代码，链接https://blog.csdn.net/Mr_health/article/details/82667722